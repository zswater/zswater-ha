...
- ✅ 一步式登录：手机号 + 密码 + 短信验证码（与营业厅登录框完全一致）
- ✅ 全程 GUI 配置，无需编辑 YAML
...
2. **登录** —— 手机号 + 密码（见下节）
3. **输入短信验证码** —— 营业厅会向该手机号发送验证码
4. **选择户号** —— 勾选要接入 HA 的户号（可多选）

### 登录方式

集成只提供**一条**登录路径，它与营业厅自己的登录框逐字段对应：

| 营业厅登录框 | 集成里的字段 | 对应接口 |
|---|---|---|
| 手机号 | 手机号 | — |
| 获取手机验证码（按钮） | 提交上一步即触发 | `GET /iwater/v1/usercenter/nt/sendAuthCode/v4.json`（`type=2`）|
| 手机验证码 | 短信验证码 | — |
| 密码 | 密码 | — |
| 登录（按钮） | 提交验证码即登录 | `POST /iwater/nt/wt/login.json` |

流程：

1. 填**手机号**和**密码**并提交 → 营业厅向该手机号发送短信验证码
2. 填入收到的**短信验证码**并提交 → 登录成功，进入户号选择

> **注意**：密码是在**提交验证码时**才校验的，所以密码填错也会在第二步才报错。

#### 为什么没有图形验证码 / 微信扫码 / 短信注册入口

网上营业厅的登录页**没有图形验证码**——它的前端代码里虽然留着 `captchaUrl` 状态，
但那是一句无效代码（`this.state.captchaUrl;`），从未渲染成图片。早期版本的本集成误读了
这一点，现已按真实登录框改正。

另外两条入口在营业厅后端确实存在，但**用户拿不到所需凭证**，因此没有提供：

- **微信 `unionid` 登录**：页面的取值方式是 `location.href.split("unionid=")[1]`，
  只有当营业厅是被**公众号菜单**带参数打开时，地址栏里才会有这个值；用户无法自行查到，
  单独打开网页版更没有。
- **统一身份认证（粤省事）登录**：需要从 `tyrz.gd.gov.cn` 跳转回来的 `ticket` / `sp` 参数。

两者所需的参数与调用方式都记录在 [`CLAUDE.md`](CLAUDE.md)（`LoginType` 的文档字符串里），
日后若要支持可直接照此实现。
...
| 用途 | 方法 | 路径 |
|---|---|---|
| 登录（`code` = 短信验证码） | POST | `/iwater/nt/wt/login.json` |
| 发送短信验证码 | GET | `/iwater/v1/usercenter/nt/sendAuthCode/v4.json` |
| 微信 unionid 登录（未接入） | POST | `/iwater/nt/wt/logining.json` |
| 统一身份认证登录（未接入） | POST | `/iwater/sso/login.json` |
| 注册（未接入） | POST | `/iwater/iwaterapi/nt.json` |
...
python custom_components/zswater/zswater_client_demo.py \
    --mobile 13800000000 --send-code

# 2. 收到验证码后登录并打印户号、欠费、抄表读数
python custom_components/zswater/zswater_client_demo.py \
    --mobile 13800000000 --password '你的密码' --sms-code 123456
```
...
**这不是登录失败，也不是接口取错了。** 网上营业厅的水费接口只会返回**营业厅自己那套系统里
已绑定**的户号：

- 它调用的接口是 `POST /iwater/v1/watermeter/queryUserMeterList/v1.json`，参数就是 `{"UNID": ""}`
  —— 与本集成发出的请求**完全一致**；
- 营业厅自己的前端在拿到空列表时会提示「**您还没有绑定户号，请去绑定户号**」，本集成遇到的是
  同一种情况。

也就是说：账号能登录，但这个账号在**网页版营业厅**里还没有绑定过水表户号。

**特别注意**：手机上「中山公用水务」小程序 / APP 里能看到的户号（例如截图里的
`100239734 *海亮`）属于**另一套前端**，它有自己的一套绑定关系，**不会**自动出现在网页版营业厅里。
本集成对接的是网页版营业厅（`smartbi.zsws.com.cn`），因此需要在那里单独绑定一次。

**解决办法（一次性，约 1 分钟）**：

1. 用浏览器打开 `https://smartbi.zsws.com.cn/`，用同一个手机号登录（手机号 + 密码 + 短信验证码）
2. 进入「我的用水户」→「绑定其他户号」，填写**户号**和**户名**并按提示完成绑定
3. 回到 Home Assistant 的集成配置页，点「提交」重新检测 —— **不需要重新登录**

绑定后本集成即可读取该户号的欠费金额、上期/本期抄表读数与历史记录。

> 小程序使用的接口属于另一套前端，本项目无法接入（拿不到它的前端资源，且需要微信会话），
> 因此没有走「换成小程序接口」这条路。
...
- **登录必须同时有密码**：营业厅要求短信验证码与密码一起提交，无法只凭验证码登录
- **必须先在网上营业厅绑定户号**：见上文「常见问题」——小程序/APP 里的绑定不被网页版共享
